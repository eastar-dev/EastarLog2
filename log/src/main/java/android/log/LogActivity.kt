...
abstract class LogActivity : AppCompatActivity() {
    override fun startService(service: Intent): ComponentName? {
        Log.pc(
            ...
            "▶▶",
            javaClass,
            service.component?.shortClassName ?: service.toUri(0),
            service
        )
    ...
    override fun startActivities(intents: Array<out Intent>?) {
        Log.pc(
            ...
            "▶▶",
            javaClass,
            intents?.forEach { it.component?.shortClassName ?: it.toUri(0) },
            intents
        )
    ...
    companion object {
        internal fun onActivityResultLog(clz: Class<*>, requestCode: Int, resultCode: Int, data: Intent?) {
            Log.pm(
                if (resultCode == Activity.RESULT_OK) Log.INFO else Log.ERROR,
                "onActivityResult",
                "◀◀",
                clz,
